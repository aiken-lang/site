...
Pattern matching on a `Bool{:ak}` value is discouraged and `if / else`
...
There are many different payloads of type `Data` that one may encounter which may not necessarily fit into one big multi-constructor type in Aiken. There is a syntax that allows checking if something is of a certain type or otherwise do something else. There is an enhancement to if expressions to be able to attempt casting but without returning an error by using the provided else instead.
