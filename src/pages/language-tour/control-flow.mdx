---
title: "Control Flow"
description: "Aiken is a functional programming language for Cardano smart contracts that treats every block as an expression. Control flow is handled through if/else expressions, when/is pattern matching, the pipe operator, and recursion — there are no loops. Used to direct validator logic, destructure custom types, handle transaction scenarios, and compose operations."
difficulty: "beginner"
keywords: ["when", "is", "if", "else", "pattern matching", "pipe operator", "expect", "recursion", "expression blocks", "control flow", "no loops"]
topics: ["control-flow", "pattern-matching", "when-is", "if-else", "pipe-operator", "recursion"]
prerequisites: ["primitive-types", "custom-types"]
related: ["functions", "custom-types", "troubleshooting"]
---
