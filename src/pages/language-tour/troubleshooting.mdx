...
description: "Aiken is a functional programming language for Cardano smart contracts where the constrained on-chain execution environment makes debugging challenging. Troubleshooting tools include the trace keyword for labelling execution paths, the ? trace-if-false operator, CBOR diagnostic notation for inspecting serialised data, and expect for catching runtime failures. Used to debug validator logic during development and diagnose why transactions fail."
difficulty: "intermediate"
keywords: ["trace", "troubleshooting", "debugging", "CBOR diagnostic", "expect", "trace-if-false", "?", "error messages", "on-chain debugging"]
topics: ["troubleshooting", "debugging", "trace", "cbor-diagnostic"]
prerequisites: ["functions", "control-flow", "validators"]
related: ["tests", "bench", "validators"]
---
