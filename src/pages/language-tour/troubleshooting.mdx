...
On-chain programming can be a bit tedious and shares a lot of commonalities with embedded programming. Because the execution environment is so contrained, programs have to be optimized and usually leave little room for troubleshooting errors.
...
<Callout type="warning">
  ...
  - **Removed by default** when building your project with `aiken build`. They can be preserved using `--keep-traces`;
  - **Kept by default** when checking your project with `aiken check`. They can be left out using `--no-traces`.
...
This can be a little hard to reason about however because booleans are "blind". That is, you loose information about the original context as you evaluate complexe boolean expressions.
...
It evaluates to `False`. From just `False`, you can't really tell which branch was actually `False` in the original expression. Yet it is often useful to reason about even larger expression and troubleshoot an issue.
...
While most are pretty transparent, the use-case for `Tag` may not strike as obvious. In fact, `Tag` is used to encode custom types on-chain, starting from tag `121` for the first constructor of a data-type, `122` for the next, and so forth. What is tagged corresponds to the fields of the constructors, as a list of objects.

Let's see some more examples of diagnostics from real Aiken's values.
...
Eventually, you will need to construct compatible values for building an associated transaction. Aiken provides blueprints as build outputs to help with that. Yet you may also control some chosen values directly using `string.diagnostic` and a test:
...
test my_datum_1() {
  ...
  string.diagnostic(datum) == @"121([42, h'48656c6c6f2c20576f726c6421'])"
}
```
