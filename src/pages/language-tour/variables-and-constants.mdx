...
description: "Aiken is a functional programming language for Cardano smart contracts where all values are immutable by default. Variables are declared with let bindings inside functions, while constants are declared with const at the module level and are fully evaluated at compile-time. Used to store and reference values throughout validator logic and across modules."
difficulty: "beginner"
keywords: ["let", "const", "variables", "constants", "immutable", "bindings", "shadowing", "type annotations"]
topics: ["variables", "constants", "let-bindings", "immutability"]
prerequisites: ["primitive-types"]
related: ["primitive-types", "functions", "modules"]
---
