...
You can also introduce new bindings through the `expect` keyword.

```aiken
expect Some(foo) = my_optional_value
```

We will see more about the expect keyword when talking about [custom
types](/language-tour/custom-types#expect). For now, it is sufficient to know
that it exists.
