...
The `fail` keyword here works for both unit and property tests, with a subtle difference for property tests. Indeed, a property that is expected to fail will run until it does or until it reaches the maximum number of tries (default to 100, like successes). If the test runner finds 100 tests that satisfy a property expected to fail, the entire test is considered a failure. Said differently, it doesn't stop at the first _success_.
