...
`Data`. Extracting from `Data` however requires the use of [assert](https://aiken-lang.org/language-tour/variables-and-constants#assert)
...
`assert` is a special keyword which works like `let`, but allows to perform
...
(considering any other like a invalid program), then `assert` is the perfect
...
Another crucial use of `assert` is to turn some opaque `Data` into a concrete
