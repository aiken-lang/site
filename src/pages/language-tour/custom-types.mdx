...
These two notations (with or without the constructor) are synonym. With the
...
*expr* is{:ak}` keywords. This syntax allows to inspect a value following the
...
otherwise. In many situation, it is better to list all patterns explicitly.
...
In the function above, you can see how we authorize any administror or any
...
`Data`. Extracting from `Data` however requires the use of
[`expect{:ak}`](./control-flow#expect) or [`if *pattern*
is{:ak}`](./control-flow#if-is) as explained in the next section.

This implicit conversion is handy for interacting with builtins that operate on
raw `Data`. Any function that accepts `Data` will automatically work with any
custom type.
...
Finally, it is also possible define mere aliases for types. A type alias lets
you create a name which is identical to another type, without any additional
information.
