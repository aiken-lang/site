...
and now to V3. This document seeks to be a reference to these design patterns and practices.
...
#### NFT example:
An NFT (Non-Fungible Token) must be created using a one-shot minting policy that ensures each minted value
...
To create an NFT minting policy, first define OutputReference and AssetName as parameters and set the action
type to mint or burn the token based on the value provided in the redeemer.
...
validator(utxo_ref: OutputReference, token_name: AssetName) {
  fn nft(action: Action, ctx: ScriptContext) -> Bool {
      ...
      expect [(asset_name, amount)] = mint |> from_minted_value |> tokens(policy_id) |> to_list() 
...
validator(utxo_ref: OutputReference, token_name: AssetName) {
  fn nft(action: Action, ctx: ScriptContext) -> Bool {
      ...
      when action is {
        ...
        Burning -> (-1 == amount)? && (asset_name == token_name)? // No need to check if output is consumed for burning
      }
    }
  }
}
...
validator {
  fn receipt(_r: Data, ctx: ScriptContext) -> Bool {
    ...
    // Select the first input and concatenate its output reference's index and tx_id hash, converting it to a byte array
    expect [Pair(asset_name, amount)] =
      mint |> from_minted_value |> tokens(policy_id) |> to_pairs()
    expect Some(first_input) = list.at(inputs, 0)
    let first_input_hash = first_input.output_reference.transaction_id.hash
    let first_input_index =
      to_bytearray(from_int(first_input.output_reference.output_index))
    let first_input_out_ref = concat(first_input_hash, first_input_index)

    // Compare the asset name with the output reference as a byte array
    asset_name == first_input_out_ref && amount == 1
  }
}
```
...
This could potentially cause the double satisfaction antipattern where anyone can pay once and get every UTxO locked with the
same price or less.  
