...
validator receipt {
  ...
  mint(_r: Data, policy_id: PolicyId, self: Transaction) {
    ...
    let first_input_index = first_input.output_reference.output_index
...
validator exploitable_swap {
  spend(
    ...
    (lovelace_of(value_paid_to_benef) >= datum.price)?
  }
...
validator swap {
  spend(
        ...
        fn(o) {
          when o.datum is {
            InlineDatum(output_datum) -> {
              ...
              o.address == from_verification_key(datum.beneficiary) && own_ref == output_datum_reference
            }
            _ -> False
          }
        },
    ...
    // asked by the seller.
    let paid_for_utxo =
      list.foldl(curr_utxo_outputs, zero, fn(n, acc) { merge(n.value, acc) })

    (lovelace_of(paid_for_utxo) >= datum.price)?
  }
...
pub type HandlerParams {
  nft: PolicyId,
  operator: VerificationKeyHash,
}

validator handler(params: HandlerParams) {
  spend(
    _optional_datum: Option<Datum>,
    _redeemer: Redeemer,
    _own_ref: OutputReference,
    self: Transaction,
  ) {
