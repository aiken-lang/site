---
title: "EUTxO Crash Course"
description: "Aiken is a functional programming language for writing smart contracts on Cardano, which uses the Extended Unspent Transaction Output (EUTxO) model. EUTxO extends the basic UTxO model with datums (data attached to outputs as contract state), redeemers (user-provided arguments to unlock funds), and scripts (predicate functions that return True or False). The foundational execution model that determines how all Aiken validators process transactions and enforce on-chain logic."
difficulty: "beginner"
keywords: ["EUTxO", "UTxO", "datum", "redeemer", "script", "transaction", "inputs", "outputs", "address", "script purpose", "spend", "mint", "withdraw", "publish", "vote", "propose"]
topics: ["eutxo", "transactions", "datums", "redeemers", "script-purposes"]
prerequisites: []
related: ["getting-started", "validators", "common-design-patterns"]
---
...
<Callout type="warning">
  ...
  **digital signatures**. We, therefore, expect readers to be either familiar
  ...
  module](https://cardanofoundation.org/en/academy/course/cbca).
</Callout>
