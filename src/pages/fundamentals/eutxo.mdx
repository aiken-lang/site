...
This course will reference cryptography concepts such as **hash
...
two parts: a header and a body. The header contains meta-information about the
...
content, change the identifier. A chain is formed by including a hash of the
block body in the header and the header hash digest of the previous block.
...
A transaction is an object with an input from where it takes assets and an
output to where they are sent. Often, as is the case in Cardano, transactions
have many inputs and many outputs. In addition to inputs and outputs,
blockchain protocols often include other elements that modify different parts
of the blockchain state. The spirit is the same as in the database world. A
transaction is an all-or-nothing atomic series of commands. Either it is
_valid_, and all its changes are applied, or it isn't, and none are applied.
...
Cardano supports two kinds of assets: the main protocol currency (a.k.a. Ada);
and user-defined currencies.
...
one can utilize the assets at this particular output. It is what defines
ownership of the assets. We'll explore this very soon. Bear with us a little
more.
...
A transaction is a form indicating which post-it notes should be destroyed and
which new ones should be pinned to the wall so that there's as much value in as
there's value out. The total value should balance out but might be shuffled
differently. The blockchain state results from looking at the entire wall of
remaining post-it notes.

An output that hasn't been spent yet (i.e. is still on the wall) is called --
you guessed it -- an _unspent transaction output_, or UTxO in short.
...
outputs are removed from the available set (post-it note is destroyed), they
can only be spent once. At least, that's what the blockchain protocol makes
sure of.
