...
provided as a witness to any transaction spending from a script address, as
...
- An _address_ is made of either 2 or 3 parts: a _header_, _payment credentials_ and (optionally) _delegation credentials_;
- The _header_ describes the _type_ of address and the _network_ it is for;
- Note that the last part, the _delegation credentials_, while optional is still <u>highly recommended</u>;
...
UTxO is and what an address is made of. And we spoke a bit about scripts. In
...
Another way to look at scripts, datums and redeemers is to think of them as forming a
