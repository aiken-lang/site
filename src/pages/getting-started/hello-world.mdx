...
```ts filename="hello_world_lock.ts"
import {
  Blockfrost,
  C,
  Constr,
...
<Callout>
  ...
  ```
  deno run --allow-net --allow-read hello_world_lock.ts
  ```

  It assumes that this file (`hello_world_lock.ts`) is placed at the root of your `hello_world` folder. At this stage, your folder should looks roughly like this:
...
Let's make a new file `hello_world_unlock.ts` and copy over some of the boilerplate
from the first one.

```ts filename="hello_world_unlock.ts"
import {
  Blockfrost,
  C,
  Constr,
...
<Callout>
  ...
  ```
  deno run --allow-net --allow-read hello_world_unlock.ts
  ```
</Callout>
