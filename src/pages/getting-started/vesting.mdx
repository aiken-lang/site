...
let's increase the difficulty and write a slightly more difficult one.
...
During this tutorial, we will introduce some new concepts from Aiken, such as:

- Datum having more complex data structure (object);
- Spending script with an optional redeemer;
- Type aliases for enhanced readability;
- Use of pattern matching;
- Unit test using Aiken's built-in test framework;
- Manipulation of time on-chain through transaction validity ranges.
...
pub fn spend(datum: Datum, _redeemer: Void, ctx: ScriptContext) {
...
This generate a assets in the `assets` folder. In particular, we can find a
`script.cbor` that contains our compiled `vesting` spending validator.
...
const datum = Data.to<Datum>(
  {
   ...
   beneficiary: /* CONTENT OF 'beneficiary.addr' */ // beneficiary address - should be bech32 cardano addr
  },
  Datum,
);
