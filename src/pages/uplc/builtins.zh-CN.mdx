...
<Callout type="info">
您可以在 [aiken-lang/prelude](https://aiken-lang.github.io/prelude/aiken/builtin.html) 中找到这些内置函数的 Aiken 对应项。
</Callout>

| 内置函数                                     | 类型参数  | 项参数 (Term Args)                   | 结果                  |
| -------------------------------------------- | --------- | ------------------------------------ | --------------------- |
...
[^1]: 当谓词为 `True` 时返回第二个参数，为 `False` 时返回第三个参数。
[^2]: 每个参数对应内置数据 (builtin data) 的每个构造函数（按此顺序）：constr, map, list, integer 和 bytestring。评估将继续执行实际对应于给定项值的任何分支。
[^3]: 当列表为空时返回第二个参数，否则返回第三个参数。
[^4]: 参数分别为：公钥、消息、签名。
