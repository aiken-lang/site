...
❯ aiken check
  ...
  Downloading packages
   Downloaded 1 package in 0.91s
    ...
    Compiling aiken-lang/hello_world 0.0.0 (/Users/aiken/Documents/aiken-lang/hello_world)
...
1. We've added a manual message using the `trace` keyword. The message is simply the one passed as redeemer. With this, we can check that the value we passed as redeemer is indeed the right one.
...
In order to test our validator, we'll need to write a short test.

## Writing a test

Aiken has support for tests built-in! As you'll see shortly, tests can also serve as benchmarks since they display the exact memory and steps execution units required to run them. They also collect traces for us. Let's write a simple test which simply runs our validator. Tests are functions without arguments which returns boolean. Yet unlike functions, they are denoted with the keyword `test`. We will need a datum, a redeemer and a script context.
...
```aiken filename="validators/hello_world.ak" {6}
test hello_world_example() {
  let datum =
    Datum { owner: #"00000000000000000000000000000000000000000000000000000000" }
